...
class Auth
{
    ...
    public function login($email, $password, $remember = 0, $captcha = NULL)
    {
        ...
        if ($block_status == "verify") {
            if ($this->checkCaptcha($captcha) == false) {
                $return['message'] = $this->lang["user_verify_failed"].'1';
        ...
        if ($block_status == "block") {
            $return['message'] = $this->lang["user_blocked"].'2';
            return $return;
        }
        ...
        $return['error'] = false;
        $return['message'] = $this->lang["logged_in"].'10';
    ...
    */
    ...
    public function activate($key)
    {
        ...
        if ($getRequest['error'] == 1) {
            $return['message'] = $getRequest['message'].'20';
    ...
    */
    ...
    {
       $query = $this->dbh->prepare("SELECT user_id as id FROM {$this->config->table_users} WHERE email = '".$email."'");		
       $query->execute();
		if(!$row = $query->fetch(\PDO::FETCH_ASSOC)) {
            return false;	
        }
    ...
    */
    ...
    public function deleteUser($uid, $password, $captcha = NULL)
    {
        ...
        if ($validatePassword['error'] == 1) {
            $this->addAttempt();
            $return['message'] = $validatePassword['message'].'y';
		...
		echo"<br>".$this->config->table_requests;

        $query = $this->dbh->prepare("DELETE FROM {$this->config->table_requests} WHERE uid = ?");

        if (!$query->execute(array($uid))) {
            $return['message'] = $this->lang["system_error"] . " #07".$this->config->table_requests;
    ...
    */
    ...
    public function resetPass($key, $password, $repeatpassword, $captcha = NULL)
    {
        ...
        if ($validatePassword['error'] == 1) {
            $return['message'] = $validatePassword['message'].'rr';
            return $return;
        }
